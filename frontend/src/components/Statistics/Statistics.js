...
function Statistics({ company, allSales }) {
  ...
  if (select == 999) {
    ...
    DateFilterSales = cSales.filter(
        (item) => (item.date.split("-").reverse().join(""))>(date.split("-").reverse().join("")) 
      );
  }
  if(company=="BGS"){console.log(DateFilterSales);}
  const grades = [];
            ...
            return (
              <>
                <div className="stats__box__row">
                  ...
                  <p className="stats__box__heading">
  ...
  const calculateAvg3 = (grade) => {
    ...
    if (sales.length > 0) {
      ...
      if (sortedSales.length === 1) return "$" + sortedSales[0].sold;
  ...
  const companyHeading = () => {
    // if (company === "PSA")
      return (
        <div className="stats__heading">
          ...
          <div className="stats__heading__time">{timeLimitButton()}</div>
        </div>
      );
    // else if (company === "CGC")
    //   return (
    //     <div className="stats__heading">
    //       <div className="stats__heading__cgc">CGC</div>
    //       <div className="stats__heading__time">{timeLimitButton()}</div>
    //     </div>
    //   );
    // else if (company === "BGS")
    //   return (
    //     <div className="stats__heading">
    //       <div className="stats__heading__bgs">BGS</div>
    //       <div className="stats__heading__time">{timeLimitButton()}</div>
    //     </div>
    //   );
  };
        ...
        <div className="stats__container">
          ...
          <div className="stats__box">
            <div className="stats__box__headings">
              ...
              <p className="stats__box__heading">Date</p>
              <p className="stats__box__heading__avg">
                {"Avg { Past 3 Sold }"}
              </p>
            </div>
          ...
          <div className="percentage__return__section">
            <div className="percentage__return__heading">Percentage Return</div>
            <div className="percentage__return">{percentageReturn()}</div>
          </div>
        </div>
      </div>
      <div className="psa__chart">
        <div className="chart">
          <Chart company={company} sales={DateFilterSales} />
        </div>
      </div>
    </div>
  );
